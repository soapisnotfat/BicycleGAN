...
class BaseModel(object):
    ...
    def init_data(self, opt, use_D=True, use_D2=True, use_E=True, use_vae=True):
        ...
        self.netG = model.define_g(opt.input_nc, opt.output_nc, opt.nz, opt.ngf,
        ...
        if use_D:
            self.netD = model.define_d(D_output_nc, opt.ndf,
        ...
        if use_D2:
            self.netD2 = model.define_d(D_output_nc, opt.ndf,
                                        which_model_net_d=opt.which_model_netD2,
                                        norm=opt.norm, use_sigmoid=use_sigmoid, init_type=opt.init_type, num_d_s=opt.num_Ds, gpu_ids=self.gpu_ids)
            model.print_network(self.netD2)

        # define E
        if use_E:
            self.netE = model.define_e(opt.output_nc, opt.nz, opt.nef,
                                       which_model_net_e=opt.which_model_netE,
                                       norm=opt.norm, init_type=opt.init_type, gpu_ids=self.gpu_ids,
                                       vae_like=use_vae)
            model.print_network(self.netE)
        ...
        self.criterionGAN = model.GANLoss(mse_loss=not use_sigmoid, tensor=self.Tensor)
        ...
        if opt.isTrain:
            ...
            for optimizer in self.optimizers:
                self.schedulers.append(model.get_scheduler(optimizer, opt))
            # st()
