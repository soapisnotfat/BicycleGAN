...
training step and implementation detail section in README might be helpful.
"""
...
class Solver:
    def __init__(self, root='dataset/edges2shoes', result_dir='result', weight_dir='weight', load_weight=False,
        ...
        self.dloader, dlen = data_loader(root=root, batch_size=batch_size, shuffle=True, img_size=img_size, mode='train')

        # Data loader for test
        self.t_dloader, _ = data_loader(root=root, batch_size=test_size, shuffle=False, img_size=img_size, mode='val')
    ...
    def train(self):
        ...
        for epoch in range(self.start_epoch, self.num_epoch):
            ...
            for iters, (img, ground_truth) in enumerate(self.dloader):
                ...
                util.progress_bar(iters, len(self.dloader), 'D_loss : %f / G_GAN_loss : %f / KL_div : %f / img_recon_loss : %f / z_recon_loss : %f'
                                  % (D_loss.data[0], G_GAN_loss.data[0], KL_div.data[0], img_recon_loss.data[0], G_alone_loss.data[0]))
